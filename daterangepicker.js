...
! function($) {
    ...
    var DateRangePicker = function(element, options, cb) {
        ...
        if (hasOptions) {
            ...
            if (typeof options.startDate == 'string') this.startDate = Date.parse(options.startDate, this.format);

            if (typeof options.endDate == 'string') this.endDate = Date.parse(options.endDate, this.format);

            if (typeof options.minDate == 'string') this.minDate = Date.parse(options.minDate, this.format);

            if (typeof options.maxDate == 'string') this.maxDate = Date.parse(options.maxDate, this.format);
        ...
        if (typeof options == 'undefined' || typeof options.ranges == 'undefined') this.container.find('.calendar')
            .show();
    ...
    DateRangePicker.prototype = {
        ...
        updateView: function() {
            ...
            this.container.find('input[name=daterangepicker_end]')
                .val(this.endDate.toString(this.format));
        ...
        move: function(node) {
            var node = (node) ? node : this.element;
            if (this.opens == 'left') {
                this.container.css({
                    top: node.offset()
        ...
        show: function(e) {
            if (e) {
                ...
                if(this.trigger.length > 0) {
                    var node = false,
                        target = $(e.target),
                        id = '#' + target.attr('id'),
                        bubble = '#' + target.parent('div').attr('id');

                    if (this.trigger.indexOf(id) > -1) {
                        node = target;
                        this.container.removeClass('start').addClass('end');
                    } else if (this.trigger.indexOf(bubble) > -1) {
                        node = target.parent('div');
                        this.container.removeClass('start').addClass('end');
                    } else {
                        this.container.removeClass('end').addClass('start');
                    }
                }
            }
            ...
            $(document)
                .on('mousedown', $.proxy(this.hide, this));
        },

        hide: function(e) {
            this.container.hide();
            $(document)
                .off('mousedown', this.hide);
        ...
        enterRange: function(e) {
            ...
            if (label == this.locale.customRangeLabel) {
                ...
                this.container.find('input[name=daterangepicker_start]')
                    .val(dates[0].toString(this.format));
                this.container.find('input[name=daterangepicker_end]')
                    .val(dates[1].toString(this.format));
            }
        },
        ...
        enterDate: function(e) {
            ...
            if (cal.hasClass('left')) {
                this.container.find('input[name=daterangepicker_start]')
                    .val(this.leftCalendar.calendar[row][col].toString(this.format));
            } else {
                this.container.find('input[name=daterangepicker_end]')
                    .val(this.rightCalendar.calendar[row][col].toString(this.format));
            }
        ...
        clickDate: function(e) {
            ...
            if(this.trigger.length > 0) this.hide();
            this.updateCalendars();
        },
        ...
        updateCalendars: function() {
            ...
            this.container.find('.calendar.right')
                .html(this.renderCalendar(this.rightCalendar.calendar, this.endDate, this.startDate, this.maxDate));
        ...
        buildCalendar: function(month, year) {
            ...
            var lastMonth = firstDay.clone()
                .add(-1)
                .day()
                .getMonth();
            var lastYear = firstDay.clone()
                .add(-1)
                .day()
                .getFullYear();
            ...
            for (var i = 0, col = 0, row = 0; i < 42; i++, col++, curDate = curDate.clone()
                .add(1)
                .day()) {
                if (i > 0 && col % 7 == 0) {
                    col = 0;
        ...
        renderCalendar: function(calendar, selected, minDate, maxDate) {
            ...
            for (var row = 0; row < 6; row++) {
                ...
                // add week number
                if (this.showWeekNumbers) html += '<td class="week">' + calendar[row][0].getWeek() + '</td>';
                ...
                for (var col = 0; col < 7; col++) {
                    ...
                    if ((minDate && calendar[row][col] < minDate) || (maxDate && calendar[row][col] > maxDate)) {
                        ...
                        cname += ' active ';
                        if (calendar[row][col].equals(this.startDate)) {
                            cname += ' start-date ';
                        }
                        if (calendar[row][col].equals(this.endDate)) {
                            cname += ' end-date ';
                        }
                    ...
                    html += '<td class="' + cname.replace(/\s+/g, ' ')
                        .replace(/^\s?(.*?)\s?$/, '$1') + '" title="' + title + '">' + calendar[row][col].getDate() + '</td>';
                }
                html += '</tr>';
            }
